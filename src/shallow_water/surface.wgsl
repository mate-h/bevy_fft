...
struct ShallowWaterMaterialUniform {
    height_scale: f32,
    tile_world_size: f32,
    grid_size: f32,
    _pad0: f32,
...
@group(#{MATERIAL_BIND_GROUP}) @binding(100) var<uniform> sw_ext: ShallowWaterMaterialUniform;
@group(#{MATERIAL_BIND_GROUP}) @binding(101) var bed_water_texture: texture_2d<f32>;
@group(#{MATERIAL_BIND_GROUP}) @binding(102) var bed_water_sampler: sampler;

/// Texel-centered UV like `ocean.wgsl` so linear sampling lines up with the sim grid.
fn sw_uv_texel(mesh_uv: vec2<f32>) -> vec2<f32> {
    let n_tex = max(sw_ext.grid_size, 1.0);
    return mesh_uv * (n_tex - 1.0) / n_tex + vec2<f32>(0.5 / n_tex);
}

fn sw_surface_height_at_uv(mesh_uv: vec2<f32>) -> f32 {
    let uv_tex = sw_uv_texel(clamp(mesh_uv, vec2<f32>(0.0), vec2<f32>(1.0)));
    let bw = textureSampleLevel(bed_water_texture, bed_water_sampler, uv_tex, 0.0).xy;
    return (bw.x + bw.y) * sw_ext.height_scale;
}

/// World-space dh/dx and dh/dz from filtered height; matches smooth fragment normals.
fn sw_world_height_gradients(mesh_uv: vec2<f32>) -> vec2<f32> {
    let n_tex = max(sw_ext.grid_size, 1.0);
    let du = 1.0 / n_tex;
    let dv = du;
    let tile = max(sw_ext.tile_world_size, 1e-6);
    let h_l = sw_surface_height_at_uv(mesh_uv - vec2<f32>(du, 0.0));
    let h_r = sw_surface_height_at_uv(mesh_uv + vec2<f32>(du, 0.0));
    let h_d = sw_surface_height_at_uv(mesh_uv - vec2<f32>(0.0, dv));
    let h_u = sw_surface_height_at_uv(mesh_uv + vec2<f32>(0.0, dv));
...
fn sw_displace_surface(instance_index: u32, position_local: vec3<f32>, mesh_uv: vec2<f32>) -> DisplacedSurface {
    let surface = sw_surface_height_at_uv(mesh_uv);
    let dhd = sw_world_height_gradients(mesh_uv);
    let deta_dx = dhd.x;
    let deta_dz = dhd.y;
...
fn sw_highres_world_normal(instance_index: u32, mesh_uv: vec2<f32>) -> vec3<f32> {
    let dhd = sw_world_height_gradients(mesh_uv);
...
fn sw_shoreline_albedo(mesh_uv: vec2<f32>, base_color: vec4<f32>) -> vec4<f32> {
    let uv_tex = sw_uv_texel(mesh_uv);
    let bw = textureSampleLevel(bed_water_texture, bed_water_sampler, uv_tex, 0.0).xy;
    let water_h = bw.y * sw_ext.height_scale;
    let shallow = exp(-water_h * 120.0);
    let bed_wet = 1.0 - exp(-2.5 * bw.x * sw_ext.height_scale);
    let sand = vec3<f32>(0.52, 0.46, 0.32);
    let rgb = mix(base_color.rgb, sand, shallow * (0.2 + 0.45 * bed_wet));
...
fn vertex(vertex: Vertex) -> VertexOutput {
    let surf = sw_displace_surface(vertex.instance_index, vertex.position, vertex.uv);
...
fn fragment(in: FragVertexOutput, @builtin(front_facing) is_front: bool) -> FragmentOutput {
    ...
    let n_world = sw_highres_world_normal(in.instance_index, in.uv);
    pbr_input.world_normal = prepare_world_normal(n_world, double_sided, is_front);
    pbr_input.N = normalize(pbr_input.world_normal);
    pbr_input.material.base_color = sw_shoreline_albedo(in.uv, pbr_input.material.base_color);
#endif
#endif
