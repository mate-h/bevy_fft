...
@group(#{MATERIAL_BIND_GROUP}) @binding(100) var<uniform> ew_ext: EwaveMaterialUniform;
@group(#{MATERIAL_BIND_GROUP}) @binding(101) var h_phi_texture: texture_2d<f32>;
@group(#{MATERIAL_BIND_GROUP}) @binding(102) var h_phi_sampler: sampler;

/// Texel-centered UV like `ocean.wgsl` so linear sampling lines up with the sim grid.
fn ew_uv_texel(mesh_uv: vec2<f32>) -> vec2<f32> {
    let n_tex = max(ew_ext.grid_size, 1.0);
    return mesh_uv * (n_tex - 1.0) / n_tex + vec2<f32>(0.5 / n_tex);
}

fn ew_surface_height_at_uv(mesh_uv: vec2<f32>) -> f32 {
    let uv_tex = ew_uv_texel(clamp(mesh_uv, vec2<f32>(0.0), vec2<f32>(1.0)));
    let hp = textureSampleLevel(h_phi_texture, h_phi_sampler, uv_tex, 0.0).xy;
    return hp.x * ew_ext.height_scale;
}

/// World-space dh/dx and dh/dz from filtered height; matches smooth fragment normals.
fn ew_world_height_gradients(mesh_uv: vec2<f32>) -> vec2<f32> {
    let n_tex = max(ew_ext.grid_size, 1.0);
    let du = 1.0 / n_tex;
    let dv = du;
    let tile = max(ew_ext.tile_world_size, 1e-6);
    let h_l = ew_surface_height_at_uv(mesh_uv - vec2<f32>(du, 0.0));
    let h_r = ew_surface_height_at_uv(mesh_uv + vec2<f32>(du, 0.0));
    let h_d = ew_surface_height_at_uv(mesh_uv - vec2<f32>(0.0, dv));
    let h_u = ew_surface_height_at_uv(mesh_uv + vec2<f32>(0.0, dv));
...
fn ew_displace_surface(instance_index: u32, position_local: vec3<f32>, mesh_uv: vec2<f32>) -> DisplacedSurface {
    let surface = ew_surface_height_at_uv(mesh_uv);
    let dhd = ew_world_height_gradients(mesh_uv);
    let deta_dx = dhd.x;
    let deta_dz = dhd.y;
...
fn ew_highres_world_normal(instance_index: u32, mesh_uv: vec2<f32>) -> vec3<f32> {
    let dhd = ew_world_height_gradients(mesh_uv);
...
fn ew_tint_albedo(_mesh_uv: vec2<f32>, base_color: vec4<f32>) -> vec4<f32> {
    return base_color;
}
...
fn vertex(vertex: Vertex) -> VertexOutput {
    let surf = ew_displace_surface(vertex.instance_index, vertex.position, vertex.uv);
...
fn fragment(in: FragVertexOutput, @builtin(front_facing) is_front: bool) -> FragmentOutput {
    ...
    let n_world = ew_highres_world_normal(in.instance_index, in.uv);
    pbr_input.world_normal = prepare_world_normal(n_world, double_sided, is_front);
    pbr_input.N = normalize(pbr_input.world_normal);
    pbr_input.material.base_color = ew_tint_albedo(in.uv, pbr_input.material.base_color);
#endif
#endif
